...
- Bilkul plain text mein jawab likhein.
- KABHI BHI markdown symbols jaise **, *, ###, ya --- istemal NA karein.
- Headers ke liye seedhe capital alfaz ya numbers (1, 2, 3) use karein.
- Saaf, asan aur pur-khaloos Roman Urdu ya English mein step-by-step likhein.
`;

function cleanFormatting(text) {
  if (!text) return '';
  return text
    .replace(/#{1,6}\s?/g, '')      // ### headings khatam
    .replace(/\*\*(.*?)\*\*/g, '$1') // **bold** khatam
    .replace(/\*(.*?)\*/g, '$1')     // *italic* khatam
    .replace(/_{1,2}(.*?)_{1,2}/g, '$1')
    .replace(/`{1,3}(.*?)`{1,3}/g, '$1')
    .replace(/---/g, '')             // horizontal line khatam
    .replace(/\n{3,}/g, '\n\n')      // extra khali lines saaf
    .trim();
}
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    // Auto Clean symbols before sending to frontend
